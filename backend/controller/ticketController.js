...
//         if (event.isCancelled) return res.status(400).json({ message: "Event is cancelled" });
//         if (event.availableTickets <= 0) return res.status(400).json({ message: "No tickets left" });
...
exports.bookTicket = async (req, res) => {
    try {
        ...
        const community = await CommunityModel.findById(event.community);
        if (community) {
            const isMember = community.members.some(m => m.user.toString() === userId);
            if (!isMember) {
                community.members.push({ user: userId, role: 'attendee' });
...
// exports.bookTicket = async (req, res) => {
//     try {
//         const { eventId, quantity: rawQty } = req.body;
//         const userId = req.user.id;
...
//         const quantity = isFreeEvent ? 1 : parseInt(rawQty || 1);

//         // Check if user already booked (for free events only)
//         if (isFreeEvent) {
//             const alreadyBooked = await TicketModel.findOne({ user: userId, event: eventId });
//             if (alreadyBooked) {
//                 return res.status(400).json({ message: "You have already claimed a free ticket for this event." });
//             }
//         }

//         // Check available tickets
//         if (event.availableTickets < quantity) {
//             return res.status(400).json({ message: `Only ${event.availableTickets} ticket(s) left.` });
//         }
...
//         const locationString = `${event.location.address}, ${event.location.city}, ${event.location.state}, ${event.location.country}${event.location.zipCode ? `, ${event.location.zipCode}` : ""}`;
//         const qrUrl = `http://localhost:5173/ticket/verify/${firstTicket._id}`;
//         const qrCode = await QRCode.toDataURL(qrUrl);
//         firstTicket.qrCode = qrCode;
...
//                             <div style="margin-bottom: 20px; border-bottom: 1px solid #e9ecef; padding-bottom: 16px;">
//                                 <span style="display: block; font-size: 12px; color: #6c757d; font-weight: 500; text-transform: uppercase;">Ticket</span>
//                                 <span style="font-size: 16px; color: #212529;">${quantity}x ${isFreeEvent ? 'Free' : 'Standard'}</span>
//                             </div>
...
//             attachments: [
//                 {
//                     filename: `ticket-${firstTicket._id}.png`,
//                     content: qrCode.split("base64,")[1],
//                     encoding: "base64",
//                     cid: "ticket_qr"
//                 }
...
exports.verifyTicket = async (req, res) => {
    ...
    try {
        const ticket = await TicketModel.findById(ticketId)
            .populate('event', 'organizer title date startTime endTime')
            .populate('user', 'name email userName profileImage');

        if (ticket.event.organizer.toString() !== userId)
